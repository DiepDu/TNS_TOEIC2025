...
namespace TNS_TOEICTest.Services
{
    public class GeminiApiKeyManager
    {
            ...
            ILogger<GeminiApiKeyManager> logger)
        {
            ...
            _connectionString = TNS.DBConnection.Connecting.SQL_MainDatabase;
            _logger = logger;
        }

        /// <summary>
        /// Lấy API key khả dụng (KHÔNG GHI LOG NGAY)
        /// </summary>
        public async Task<ApiKeyUsageResult> GetAvailableApiKeyAsync()
        {
            try
            {
                ...
                // ✅ Tính remaining quota (predicted - chưa ghi log)
                apiKey.RemainingQuota = apiKey.DailyLimit - apiKey.UsageCount;

                _logger.LogInformation(
                    "✅ Selected {KeyName} - Current usage: {Usage}/{Limit} - Remaining: {Remaining}",
                    apiKey.KeyName,
                    apiKey.UsageCount,
                    apiKey.DailyLimit,
        ...
        public async Task LogApiCallAttemptAsync(int keyId, bool isSuccess)
        {
            try
            {
                ...
                _logger.LogInformation(
                    "✅ Logged API call for KeyID {KeyID} - Success: {IsSuccess}",
                    keyId, isSuccess
                );
            }
            catch (Exception ex)
