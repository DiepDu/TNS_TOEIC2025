    ...
    public class QuestionListModel : PageModel
    {
        ...
        public IActionResult OnPostTogglePublish([FromBody] ToggleRequest request)
        {
            ...
            if (request.Publish) // Chỉ kiểm tra khi BẬT (Publish = true)
            {
                ...
                var zValidationRecord = new QuestionAccessData.Part1_Question_Info(request.QuestionKey);
                ...
                if (string.IsNullOrWhiteSpace(zValidationRecord.QuestionImage))
                {
                    errors.Add("QuestionImage không được trống.");
                }
