...
namespace TNS_TOEICPart6.Areas.TOEICPart6.Pages
{
    ...
    public class QuestionModel : PageModel
    {
        ...
        public IActionResult OnPostRecordRead([FromBody] ItemRequest request)
        {
            ...
            if (UserLogin.Role.IsRead)
            {
                QuestionAccessData.Part6_Question_Info zRecord;
                if (string.IsNullOrEmpty(request.QuestionKey) || request.QuestionKey.Length != 36)
                    zRecord = new QuestionAccessData.Part6_Question_Info();
                else
                    zRecord = new QuestionAccessData.Part6_Question_Info(request.QuestionKey);
        ...
        public IActionResult OnPostRecordCreate()
        {
            ...
            var zRecord = JsonSerializer.Deserialize<QuestionAccessData.Part6_Question_Info>(recordJson);
        ...
        public IActionResult OnPostRecordDel([FromBody] ItemRequest request)
        {
            ...
            if (UserLogin.Role.IsDelete)
            {
                ...
                {
                    QuestionAccessData.Part6_Question_Info zRecord = new QuestionAccessData.Part6_Question_Info();
        ...
        public IActionResult OnPostRecordDell([FromBody] ItemRequest request)
        {
            ...
            if (UserLogin.Role.IsDelete)
            {
                ...
                {
                    QuestionAccessData.Part6_Question_Info zRecord = new QuestionAccessData.Part6_Question_Info(request.QuestionKey);
                    if (zRecord.Status == "OK")
                    {
                        string wwwPath = _env.WebRootPath;
