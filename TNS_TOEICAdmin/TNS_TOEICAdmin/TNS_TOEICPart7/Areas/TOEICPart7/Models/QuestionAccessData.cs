...
namespace TNS_TOEICPart7.Areas.TOEICPart7.Models
{
    ...
    {
        public class Part7_Question_Info
        {
            #region [ Field Name ]
            ...
            public Part7_Question_Info()
            {
                Guid zNewID = Guid.NewGuid();
                _QuestionKey = zNewID.ToString();
                _IsNewRecord = true;
            }
            public Part7_Question_Info(string QuestionKey)
            {
                ...
                string zSQL = "SELECT * FROM [dbo].[TEC_Part7_Question] WHERE QuestionKey = @QuestionKey";
            ...
            public string Create()
            {
                ...
                string zSQL = "INSERT INTO [dbo].[TEC_Part7_Question] (QuestionKey, QuestionImage, QuestionText, QuestionVoice, SkillLevel, Ranking,Parent, Publish, RecordStatus, CreatedBy, CreatedName, ModifiedBy, ModifiedName, Category, GrammarTopic, VocabularyTopic, ErrorType) " +
                              "VALUES (@QuestionKey, @QuestionImage, @QuestionText, @QuestionVoice, @SkillLevel, @Ranking, @Parent, @Publish, @RecordStatus, @CreatedBy, @CreatedName, @ModifiedBy, @ModifiedName, @Category, @GrammarTopic, @VocabularyTopic, @ErrorType)";
                string zResult = "";
                string zConnectionString = TNS.DBConnection.Connecting.SQL_MainDatabase;
                SqlConnection zConnect = new SqlConnection(zConnectionString);
                zConnect.Open();
            ...
            {
                string zSQL = "UPDATE [dbo].[TEC_Part7_Question] SET " +
            ...
            {
                string zResult = "";
                string zSQL = "UPDATE [dbo].[TEC_Part7_Question] Set RecordStatus = 99 WHERE QuestionKey = @QuestionKey";
                string zConnectionString = TNS.DBConnection.Connecting.SQL_MainDatabase;
                SqlConnection zConnect = new SqlConnection(zConnectionString);
                zConnect.Open();
                try
            ...
            {
                string zResult = "";
                string zSQL = "DELETE FROM [dbo].[TEC_Part7_Question] WHERE QuestionKey = @QuestionKey";
                string zConnectionString = TNS.DBConnection.Connecting.SQL_MainDatabase;
                SqlConnection zConnect = new SqlConnection(zConnectionString);
                zConnect.Open();
                try
