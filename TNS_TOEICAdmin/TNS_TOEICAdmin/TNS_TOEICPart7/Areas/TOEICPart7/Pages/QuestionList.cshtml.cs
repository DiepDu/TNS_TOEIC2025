    ...
    public class QuestionListModel : PageModel
    {
        ...
        public IActionResult OnPostTogglePublish([FromBody] ToggleRequest request)
        {
            ...
            if (request.Publish) // Only validate when turning ON
            {
                ...
                var zValidationRecord = new QuestionAccessData.Part7_Question_Info(request.QuestionKey);
                ...
                // ✅ PART7: Kiểm tra QuestionText (thay vì Voice/Audio)
                if (string.IsNullOrWhiteSpace(zValidationRecord.QuestionText))
                {
                    errors.Add("Passage text cannot be empty.");
                }

                // ✅ PART7: KHÔNG kiểm tra Explanation (có thể NULL - giống Part6)
                ...
                // ✅ PART7: ValidateChildQuestions sẽ kiểm tra tối thiểu 4 câu hỏi con
            ...
            // === UPDATE STATUS ===
            var zRecord = new QuestionAccessData.Part7_Question_Info(request.QuestionKey);
