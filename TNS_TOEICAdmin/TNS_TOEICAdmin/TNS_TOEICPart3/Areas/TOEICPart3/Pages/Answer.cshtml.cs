...
namespace TNS_TOEICPart3.Areas.TOEICPart3.Pages
{
    ...
    public class AnswerModel : PageModel
    {
        ...
        public IActionResult OnPostRead([FromBody] ItemRequest request)
        {
            ...
            AnswerAccessData.Part3_Answer_Info record;
            try
            {
                if (string.IsNullOrEmpty(request.AnswerKey) || request.AnswerKey.Length != 36)
                {
                    record = new AnswerAccessData.Part3_Answer_Info(request.QuestionKey);
                    record.Message = "No existing record, initialized as new.";
                }
                else
                {
                    record = new AnswerAccessData.Part3_Answer_Info(request.AnswerKey, request.QuestionKey);
        ...
        public IActionResult OnPostUpdate([FromBody] AnswerAccessData.Part3_Answer_Info request)
        {
            CheckAuth();
        ...
        public IActionResult OnPostDelete([FromBody] ItemRequest request)
        {
            ...
            try
            {
                var record = new AnswerAccessData.Part3_Answer_Info(request.AnswerKey, null);
                record.Delete();
        ...
        {
            string sql = @"UPDATE [dbo].[TEC_Part3_Answer] 
        ...
        public IActionResult OnPostCountAnswers([FromBody] ItemRequest request)
        {
            ...
            string sql = "SELECT COUNT(*) FROM [dbo].[TEC_Part3_Answer] WHERE QuestionKey = @QuestionKey AND RecordStatus != 99";
            string connectionString = TNS.DBConnection.Connecting.SQL_MainDatabase;
            using (var conn = new SqlConnection(connectionString))
            {
                conn.Open();
